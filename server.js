...
app.get('/api/tools/categories', (req, res) => {
  db.all('SELECT DISTINCT Category FROM Tools WHERE Category IS NOT NULL', [], (err, rows) => {
    if (err) {
...
app.get('/api/tools/storage-locations', (req, res) => {
  db.all('SELECT DISTINCT Storage_Location FROM Tools WHERE Storage_Location IS NOT NULL', [], (err, rows) => {
    if (err) {
      console.error('Error fetching tool storage locations:', err);
      return res.status(500).json({ error: err.message });
    }
    
    // Extract storage locations into a simple array
...
app.post('/api/tools/import', (req, res) => {
  ...
  if (!tools || !Array.isArray(tools) || tools.length === 0) {
    return res.status(400).json({ error: 'No tool data provided or invalid format' });
  }
